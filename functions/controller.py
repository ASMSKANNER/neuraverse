...
class Controller:
            ...
            if await self.portal.privy.privy_authorize():
                ...
                if not faucet_last_claim:
                    ...
                    try:
                        ...
                        can_use_faucet = (now_ms - last_ts) >= 86_400_000
                    except Exception as e:
                        logger.error(
                            f"{self.wallet} | Failed to parse faucet_last_claim='{faucet_last_claim}': {e}"
                        )
                        can_use_faucet = True
                ...
                if wallet_balance.Ether > self.settings.min_native_balance:
                    ...
                    if await self.omnihub.is_minting():
                        if self.settings.omnihub_repeat_if_already_minted:
    ...
    async def connect_discord(self) -> bool:
        try:
            ...
            bind_discord = await self.portal.bind_discord(callback=callback, code_verifier=code_verifier)
            ...
            for quest in actionable_claimable:
                ...
                try:
                    ...
                    if not claim_result:
                        ...
                        logger.error(
                            f"{self.wallet} | Failed to claim quest: {quest_name}. Next action in {sleep_s}s"
                        )
                    else:
                        total_quest_claimed += 1
                        logger.success(
                            f"{self.wallet} | Successfully claimed quest: {quest_name}. Next action in {sleep_s}s"
                        )
            ...
            for quest in actionable_not_completed:
                ...
                try:
                    ...
                    if not completion_result:
                        ...
                        logger.error(
                            f"{self.wallet} | Failed to complete quest: {quest_name}. Next action in {sleep_s}s"
                        )
                        continue
            ...
            while completed < total_swaps and attempts < max_fail_attempts:
                ...
                try:
                    if native_balance.Ether < min_native_balance:
                        logger.warning(
                            f"{self.wallet} | Native balance low: {native_balance.Ether} ANRK (min required: {min_native_balance} ANRK)"
                        )
                        ...
                        while native_balance.Ether < min_native_balance and attempts < max_fail_attempts:
                            ...
                            logger.info(
                                f"{self.wallet} | Restoring native balance: swapping {swap_amount.Ether:.5f} {from_token.title} → ANKR"
                            )
                        ...
                        if from_token_balance_value <= 0.01:
                            ...
                            swap_amount = TokenAmount(
                                amount=safe_wei,
                                decimals=18 if from_token.address == Contracts.ANKR.address
                        ...
                        logger.info(
                            f"{self.wallet} | Swapping {swap_amount.Ether:.5f} {from_token.title} → {to_token.title}"
                        )
